...
# Function to update specific LED
...
# Function to update all LEDs in a row
    ...
    if event.pressed:
        if (row, col) == scroll_voice_up_key:
            ...
            if row < NUM_ROWS and col < SEQUENCE_LENGTH:
                step_index = col
                sequences[current_patterns[row]][row][step_index] = not sequences[current_patterns[row]][row][step_index]
                update_specific_led(row, step_index)
    ...
    if playing:
        ...
        if diff >= steps_millis:
            ...
            for i in range(NUM_VOICES):
                if sequences[current_patterns[i]][i][step_counter]:
